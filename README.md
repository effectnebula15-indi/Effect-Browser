...
Подробная пошаговая инструкция (SDK-компоненты, release-сборка со своей
подписью, установка на телефон, разбор типичных ошибок сборки) —
[docs/BUILDING.md](docs/BUILDING.md). APK не публикуется через релизы этого
репозитория: GeckoView + бинарники Tor делают файл слишком большим
(150–530 МБ на ABI) для обычных каналов доставки — собирайте локально.
