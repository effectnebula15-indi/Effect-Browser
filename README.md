...
| + | Скачивание и загрузка файлов, домашняя страница | Готово |

Проект собирается (`./gradlew assembleDebug`) и проходит юнит-тесты (`./gradlew test`, 43 теста).
...
Обе стороны обмена файлами с сайтом:

- **Скачивание.** Gecko отдаёт готовый поток ответа (`onExternalResponse`), приложение пишет
  его на диск. На Android 10+ файл уходит в системную папку «Загрузки» через MediaStore —
  разрешений не требуется, файл виден в файловом менеджере. На Android 8–9 MediaStore ещё нет,
  а запись в общую папку требовала бы `WRITE_EXTERNAL_STORAGE`, поэтому файл кладётся в
  собственный каталог приложения и открывается через `FileProvider`. По завершении внизу
  появляется плашка с кнопкой «Открыть».

  Важно: скачивание **никогда не перезапрашивает URL самостоятельно**. Обычный HTTP-клиент
  пошёл бы мимо настроек прокси Gecko, то есть для Tor-контейнера — открытым соединением с
  реального IP. Пишется только тот поток, который отдал Gecko.

- **Загрузка на сайт.** `<input type="file">` открывает системный выбор документов с учётом
  атрибута `accept` и множественного выбора.

Имя файла выводится из `Content-Disposition` (включая RFC 5987 `filename*` с не-ASCII), затем
из URL, затем из MIME-типа; путь и запрещённые символы вычищаются — сервер вправе прислать
`../../etc/passwd`. Правила закреплены тестами (`DownloadNamingTest`).

## Домашняя страница

Новая вкладка открывается на **google.com**. Для Tor-контейнеров вместо этого DuckDuckGo:
Google регулярно блокирует или показывает капчу трафику выходных узлов, и вкладка открывалась бы на
странице с ошибкой. Меняется в одном месте — `SearchEngine.homeUrlFor`.
...
├── files/         FileDownloader, DownloadNaming, FilePickerCoordinator
...
5. Отступы сверху на устройстве с вырезом под камеру: контент теперь сдвинут по
   `WindowInsets.safeDrawing`, но проверить это можно только на железе с реальным вырезом.
6. Скачивание файла (Android 10+ и отдельно Android 8–9, где путь через `FileProvider`) и
   выбор файла для `<input type="file">`.
